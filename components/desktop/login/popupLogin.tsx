                ...
                <form action="">
                  <div className="flex flex-col">
                    ...
                    {response?.success && <>
                      ...
                      <input onChange={(e) => setPassword(e.target.value)} value={password}  placeholder="**********" name="email" type="password" className={`${passwordValid? 'focus:ring-1 focus:ring-sky-100/50 focus:border-sky-400' : 'ring-1 ring-red-600 border-red-600'} border rounded-md p-1  outline-none`}/>
                      {!passwordValid && <p className="text-red-600 text-sm">This section must be filled.</p>}
                    </>}
                  </div>
                  <div className="mt-4">
                    <button onClick={handleSubmit} type="button" disabled={!response?.success} className={`${response?.success ? 'bg-orange-600 text-white' : 'bg-gray-100'} w-full p-3 rounded-full font-semibold text-gray-400`}>{response?.success ?'Log in' :'Continue'}</button>
                  </div>
                </form>
              <div className="flex">
